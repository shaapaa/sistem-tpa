...
export default function AdminDashboard() {
  ...
  return <div className="space-y-6">
    <PageHeader eyebrow="Ruang administrasi" title="Dashboard Admin" description="Kelola dan pantau kondisi operasional TPA Baitul Yatama." action={<div className="rounded-lg border border-border bg-muted/35 px-3 py-2 text-right"><p className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Hari ini</p><p className="mt-0.5 text-sm font-medium text-foreground">{formatDate(today)}</p></div>} />

    <section className="rounded-xl border border-border bg-card p-4 shadow-sm sm:p-5">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between"><div className="space-y-1"><Label className="text-[10px] text-muted-foreground">Periode presensi</Label><Select value={period} onValueChange={(value) => value && setPeriod(value)} items={PERIODS}><SelectTrigger className="h-9 w-48"><SelectValue /></SelectTrigger><SelectContent>{PERIODS.map((option) => <SelectItem key={option.value} value={option.value}>{option.label}</SelectItem>)}</SelectContent></Select></div>{period === "CUSTOM" && <><div className="space-y-1"><Label className="text-[10px] text-muted-foreground">Mulai</Label><DatePicker value={customFrom} onChange={setCustomFrom} /></div><div className="space-y-1"><Label className="text-[10px] text-muted-foreground">Selesai</Label><DatePicker value={customTo} onChange={setCustomTo} /></div></>}<div className="xl:ml-auto"><p className="mb-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">Akses cepat</p><div className="grid grid-cols-2 gap-2 sm:grid-cols-5">{[{ href: "/admin/santri", label: "Santri", icon: Users }, { href: "/admin/pengajar", label: "Pengajar", icon: GraduationCap }, { href: "/admin/kelompok", label: "Kelompok", icon: BookOpen }, { href: "/admin/jadwal", label: "Jadwal", icon: CalendarDays }, { href: "/admin/users", label: "Profil & Akun", icon: UserCog }].map((item) => <Link key={item.href} href={item.href} className="flex min-h-9 items-center gap-2 rounded-md border border-border bg-background px-3 py-2 text-xs font-medium text-foreground transition-all hover:border-primary/35 hover:shadow-sm"><span className="rounded-md bg-primary/10 p-1 text-primary"><item.icon className="h-3.5 w-3.5 shrink-0" /></span>{item.label}</Link>)}</div></div></div>
      {!rangeValid && <p className="mt-3 text-sm text-destructive">Tanggal mulai tidak boleh setelah tanggal selesai.</p>}
    </section>

    <section className="grid grid-cols-2 gap-3 lg:grid-cols-4"><StatCard label="Santri Aktif" value={santriAktif} detail="mengikuti pembelajaran" icon={Users} className="border-sky-200 bg-sky-50/80 text-sky-900 hover:bg-sky-100/80" href="/admin/santri" /><StatCard label="Pengajar Aktif" value={pengajarAktif} detail="master dengan akun aktif" icon={GraduationCap} className="border-violet-200 bg-violet-50/80 text-violet-900 hover:bg-violet-100/80" href="/admin/pengajar" /><StatCard label="Kelompok Aktif" value={kelompokAktif} detail="kelompok tersedia" icon={BookOpen} className="border-emerald-200 bg-emerald-50/80 text-emerald-900 hover:bg-emerald-100/80" href="/admin/kelompok" /><StatCard label="Akun Aktif" value={akunAktif} detail="akses sistem aktif" icon={UserCog} className="border-amber-200 bg-amber-50/80 text-amber-900 hover:bg-amber-100/80" href="/admin/users" /></section>
    ...
    <section className="overflow-hidden rounded-xl border border-border border-l-4 border-l-primary bg-card p-5 shadow-sm sm:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><SectionHeader title={jadwalDate === today ? "Operasional hari ini" : "Operasional sesi"} description={jadwalDateValid ? `Jadwal berulang untuk ${formatDate(jadwalDate)}.` : "Pilih tanggal referensi dalam periode."} /><div className="w-full sm:w-52"><Label className="text-[10px] text-muted-foreground">Tanggal referensi jadwal</Label><div className="mt-1"><DatePicker value={jadwalDate} onChange={setJadwalDate} /></div></div></div>{!jadwalDateValid ? <div className="mt-5"><EmptyState message="Tanggal jadwal harus berada dalam periode yang dipilih." /></div> : periodLoading ? <div className="mt-5 grid gap-3 md:grid-cols-2">{Array.from({ length: 2 }).map((_, index) => <div key={index} className="h-40 animate-pulse rounded-xl bg-muted" />)}</div> : periodError ? <p className="mt-5 text-sm text-destructive">{periodError}</p> : jadwal.length === 0 ? <div className="mt-5"><EmptyState message="Belum ada jadwal untuk tanggal ini." hint="Jadwal mengikuti hari dalam minggu dari tanggal referensi." /></div> : <div className="mt-5 grid gap-3 md:grid-cols-2">{jadwal.map((item) => <ScheduleItem key={item.id} item={item} santriCount={santriPerSesi[item.sesi?.id ?? ""] ?? 0} />)}</div>}</section>
  </div>
}
