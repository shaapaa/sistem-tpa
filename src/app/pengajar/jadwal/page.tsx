...
export default function PengajarJadwalPage() {
  ...
  useEffect(() => { const load = async () => { if (!user) return; const { data: pengajar } = await supabase.from("pengajar").select("id").eq("profile_id", user.id).single(); if (!pengajar) { setLoading(false); return }; const { data } = await supabase.from("jadwal_sesi").select("id, hari, jam_mulai, jam_selesai, sesi(nama), jadwal_sesi_pengajar!inner(pengajar_id)").eq("jadwal_sesi_pengajar.pengajar_id", pengajar.id).order("hari").order("jam_mulai"); setJadwals((data ?? []) as unknown as Jadwal[]); setLoading(false) }; void load() }, [user])
